...
import { Bell, BookOpen, Award, MessageCircle, TrendingUp, CheckCheck, Trash2 } from "lucide-react";
...
const getColorForType = (type: string) => {
  switch (type) {
    ...
    case "certificate": return "bg-accent/10 text-accent";
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">{unreadCount} unread notification{unreadCount !== 1 ? "s" : ""}</p>
          </div>
          {unreadCount > 0 && (
        ...
        {notifications.length === 0 ? (
          <Card className="shadow-card border-dashed border-2">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="font-display text-lg font-semibold text-foreground mb-1">No notifications</h3>
              ...
              >
                ...
                >
                  <CardContent className="p-4 flex items-start gap-4">
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center shrink-0 ${getColorForType(n.type)}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {n.read !== true && <span className="h-2 w-2 rounded-full bg-accent shrink-0" />}
                        </div>
                        <Button
                          variant="ghost"
                          size="icon"
                      ...
                      <p className="text-xs text-muted-foreground/60 mt-1.5">{formatTime(n.createdAt)}</p>
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
