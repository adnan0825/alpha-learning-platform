...
const BrowseCourses: React.FC = () => {
  ...
  const enroll = async (courseId: string) => {
    ...
    try {
      ...
      toast({ 
        title: err.message || "Enrollment failed", 
  ...
  return (
    <>
      <div className="space-y-8 animate-fade-in">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">Browse Courses</h1>
          <p className="text-muted-foreground">Discover new courses and start learning</p>
        </div>
            ...
            {filtered.map((course) => (
              <div key={course.id}>
                <CourseCard
                  course={course}
                  onClick={() => {
                    console.log("Card clicked:", course.id, "Enrolled:", enrolledIds.has(course.id));
                    handleCourseClick(course);
                  }}
                />
                {enrolledIds.has(course.id) && <p className="mt-2 text-xs text-center text-success font-medium">✓ Enrolled</p>}
              </div>
            ))}
          </div>
        ) : (
