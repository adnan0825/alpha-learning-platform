  ...
  return (
    <>
      <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
        ...
        >
          <ArrowLeft size={16} /> {t("quiz.backCourse")}
        </button>
        ...
        {displayResult ? (
          <Card className="shadow-card">
            <CardContent className="p-8 text-center">
              <Trophy
                size={48}
                className={`mx-auto mb-4 ${displayResult.score >= displayResult.total * 0.7 ? "text-amber-500" : "text-muted-foreground"}`}
              />
              ...
              <p
                className={`text-sm font-medium ${displayResult.score >= displayResult.total * 0.7 ? "text-success" : "text-destructive"}`}
              >
                {displayResult.score >= displayResult.total * 0.7
                  ? `🎉 ${t("quiz.passed")}`
                  : t("quiz.tryAgain")}
              </p>
              ...
              <div className="mt-6 space-y-3 text-left">
                {quiz.questions.map((q, i) => {
                  ...
                  return (
                    <div
                      key={q.id}
                      className={`p-3 rounded-lg border ${isCorrect ? "border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-900" : "border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900"}`}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            {t("quiz.yourAnswer")}:{" "}
                            {q.options[displayResult.answers[i]] ||
                              t("quiz.notAnswered")}
                            {!isCorrect && (
          ...
          <>
            ...
            <Button
              ...
              className="w-full gradient-accent text-accent-foreground font-semibold h-12 hover:opacity-90"
            >
