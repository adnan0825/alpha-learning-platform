...
import { FileQuestion, Plus, Trash2, CheckCircle, BookOpen } from "lucide-react";
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground flex items-center gap-2">
              <FileQuestion size={24} className="text-accent" /> Quiz Manager
            </h1>
            <p className="text-muted-foreground text-sm">Create and manage quizzes for your courses</p>
          </div>
          <Button onClick={() => setShowCreate(!showCreate)} className="gradient-accent text-accent-foreground hover:opacity-90">
            <Plus size={16} className="mr-1.5" /> Create Quiz
          </Button>
        </motion.div>
            ...
            <Card className="shadow-card border-accent/20">
              <CardContent className="p-6 space-y-5">
                <div className="space-y-2">
                  ...
                  <Input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="e.g. Chapter 1 Review Quiz" className="bg-muted/30" />
                </div>

                {questions.map((q, qi) => (
                  <div key={qi} className="p-4 rounded-xl border border-border/50 space-y-3">
                    ...
                    <Input value={q.question} onChange={e => updateQuestion(qi, "question", e.target.value)} placeholder="Enter question..." className="bg-muted/30" />
                        ...
                        <div key={oi} className="flex items-center gap-2">
                          <button
                            ...
                            className={`h-5 w-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                              q.correctIndex === oi ? "border-success bg-success text-success-foreground" : "border-border"
                            }`}
                          >
                            {q.correctIndex === oi && <CheckCircle size={10} />}
                          </button>
                          <Input value={opt} onChange={e => updateOption(qi, oi, e.target.value)} placeholder={`Option ${oi + 1}`} className="bg-muted/30 text-sm h-8" />
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button onClick={createQuiz} className="gradient-accent text-accent-foreground hover:opacity-90">Create Quiz</Button>
                </div>
              </CardContent>
            </Card>
          </motion.div>
          ...
          {quizzes.length === 0 ? (
            <Card className="shadow-card border-dashed border-2">
              <CardContent className="flex flex-col items-center justify-center py-12">
                <FileQuestion size={40} className="text-muted-foreground/30 mb-3" />
                <p className="text-sm text-muted-foreground">No quizzes for this course yet</p>
              </CardContent>
            </Card>
          ) : quizzes.map((quiz, i) => (
            <motion.div key={quiz.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
