...
function issueTokenPayload(user: {
  id: number;
  email: string;
  role: string;
  name: string;
...
async function resolveRoleForNewUser(): Promise<"admin" | "student"> {
  const result = await query("SELECT COUNT(*)::int AS total FROM users");
  return Number(result.rows[0]?.total ?? 0) === 0 ? "admin" : "student";
}
...
export const register = async (
  ...
  const assignedRole = await resolveRoleForNewUser();
  ...
  const result = await query(
    ...
    [normalizedEmail, hashedPassword, name, assignedRole],
  );
 ...
 */
export const loginWithGoogleAuthCode = async (
  code: string,
  redirectUri: string,
) => {
  const clientId = process.env.GOOGLE_CLIENT_ID;
