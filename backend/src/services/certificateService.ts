...
export async function autoIssue(
  ...
  if (existingResult.rows.length > 0) {
    return { certificate: mapRow(existingResult.rows[0]), alreadyExisted: true };
  }

  // Look up course title and instructor name
  const courseResult = await pool.query(
    `SELECT c.title, u.display_name as instructor_name
     FROM courses c
  ...
  const { title: courseTitle, instructor_name: instructorName } = courseResult.rows[0];
  ...
  const studentName = studentResult.rows[0].display_name ?? '';
  ...
  const insertResult = await pool.query(
    `INSERT INTO certificates
     ...
     RETURNING *`,
    [studentId, courseId, certificateNumber, instructorName, studentName, verificationUrl]
  );
  ...
  try {
    await pool.query(
      ...
      [studentId, `Congratulations! Your certificate for ${courseTitle} is ready.`]
    );
  } catch (notifError) {
...
export async function getByNumber(certificateNumber: string): Promise<CertificateRow | null> {
