...
function pickCourseUpdateBody(body: unknown): Parameters<typeof courseService.updateCourse>[1] {
...
router.put('/:id', authenticate, authorize('instructor', 'admin'), async (req: AuthRequest, res: Response) => {
  try {
    const course = await courseService.updateCourse(parseInt(req.params.id), pickCourseUpdateBody(req.body));
    if (!course) {
      return res.status(404).json({ error: 'Course not found or nothing to update' });
    }
    res.json(course);
  } catch (error: any) {
...
router.get('/:id/modules', async (req: AuthRequest, res: Response) => {
  try {
    const modules = await courseService.getCourseModules(parseInt(req.params.id));
    res.json(modules);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
