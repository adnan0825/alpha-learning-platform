...
const passwordAuthAllowed = () => process.env.AUTH_ALLOW_PASSWORD_LOGIN === 'true';

router.post('/signup', async (req: AuthRequest, res: Response) => {
  if (!passwordAuthAllowed()) {
    return res.status(403).json({ error: 'Email/password sign-up is disabled. Use Google Sign-In.' });
  }
  try {
...
router.post('/login', async (req: AuthRequest, res: Response) => {
  if (!passwordAuthAllowed()) {
    return res.status(403).json({ error: 'Password sign-in is disabled. Use Google Sign-In.' });
  }
  try {
...
router.post('/google/code', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const msg = error.message || 'Google sign-in failed';
    const status =
      msg.includes('not configured') || msg.includes('GOOGLE_CLIENT_SECRET') ? 503 : 401;
    res.status(status).json({ error: msg });
  }
});
...
router.put('/profile', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const user = await authService.updateProfile(req.user!.id, { name, bio, avatar });
    res.json(user);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
