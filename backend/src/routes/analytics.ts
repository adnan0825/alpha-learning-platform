...
router.post('/bookmarks/:courseId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
    const bookmark = await analyticsService.addBookmark(req.user!.id, courseId);
...
router.delete('/bookmarks/:courseId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
    const result = await analyticsService.removeBookmark(req.user!.id, courseId);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/bookmarks/:courseId/check', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
    const isBookmarked = await analyticsService.isBookmarked(req.user!.id, courseId);
...
router.get('/notifications', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const notifications = await analyticsService.getNotifications(req.user!.id, unreadOnly);
...
router.get('/notifications/count', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const count = await analyticsService.getUnreadNotificationCount(req.user!.id);
    res.json({ count });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.put('/notifications/:id/read', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const notification = await analyticsService.markNotificationAsRead(req.user!.id, notificationId);
...
router.put('/notifications/read-all', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const result = await analyticsService.markAllNotificationsAsRead(req.user!.id);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/notifications/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const notificationId = parseInt(req.params.id);
    const result = await analyticsService.deleteNotification(req.user!.id, notificationId);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.put('/notes/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { content } = req.body;
    const noteId = parseInt(req.params.id);
    const note = await analyticsService.updateNote(req.user!.id, noteId, content);
    res.json(note);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/announcements/course/:courseId', async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
...
router.put('/announcements/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.delete('/announcements/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const announcementId = parseInt(req.params.id);
    const result = await analyticsService.deleteAnnouncement(req.user!.id, announcementId);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/learning-paths/:id/courses', async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.id);
...
router.post('/learning-paths', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.put('/learning-paths/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.id);
    const { title, description, isActive } = req.body;
    const path = await analyticsService.updateLearningPath(pathId, title, description, isActive);
    res.json(path);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/learning-paths/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.id);
...
router.post('/learning-paths/:pathId/courses', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.pathId);
    const { courseId, position } = req.body;
    const result = await analyticsService.addCourseToPath(pathId, parseInt(courseId), position || 0);
...
router.delete('/learning-paths/:pathId/courses/:courseId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.pathId);
    const courseId = parseInt(req.params.courseId);
    const result = await analyticsService.removeCourseFromPath(pathId, courseId);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/user/learning-paths', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/user/learning-paths/:pathId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.pathId);
    const result = await analyticsService.enrollInLearningPath(req.user!.id, pathId);
...
router.put('/user/learning-paths/:pathId/progress', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const pathId = parseInt(req.params.pathId);
    const { progress } = req.body;
    const result = await analyticsService.updateLearningPathProgress(req.user!.id, pathId, progress);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/instructor/revenue', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = req.query.courseId ? parseInt(req.query.courseId as string) : undefined;
    const revenue = await analyticsService.getInstructorRevenue(req.user!.id, courseId);
    res.json(revenue);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/instructor/stats', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/instructor/quiz-stats/:courseId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
...
router.get('/instructor/grades/:courseId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const courseId = parseInt(req.params.courseId);
    const grades = await analyticsService.getStudentGrades(req.user!.id, courseId);
    res.json(grades);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/growth-trends', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { query } = await import('../config/db');
    ...
    const growthData = await query(`
      ...
      user_counts AS (
        SELECT
          ...
          (SELECT COUNT(*) FROM enrollments WHERE created_at >= m.month_start AND created_at < m.month_start + INTERVAL '1 month') as new_enrollments
        FROM months m
      )
      SELECT
...
router.get('/category-distribution', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { query } = await import('../config/db');
